        ...
        if (impacto_ban) {
          ...
          if (schemeProyect.length > 0) {
            schemeProyect.forEach((ps) => {
              ...
              if (ps['distance_init'] == null) {
                ...
                internacional =
                  value_transport[0]['valor'] * ps['distance_init'];
              }
          ...
          if (listaACR.length > 0) {
            let consumos = this.ECDList.filter(
              (ecd) =>
                ecd['annual_consumption_required_id'] == listaACR[0]['id']
            );
          ...
          //C1
          let ECDPs = this.ECDPList.filter(
            (c) => c['project_id'] == idProyecto
          );
          if (ECDPs.length > 0) {
            ECDPs.forEach((ECDP) => {
              let energia = this.SIDList.filter(
                (sid) =>
                  sid['sourceInformarion_id'] ==
                    ECDP['source_information_id'] &&
                  sid['potential_type_id'] == impacto['id']
              );
              resultado_impacto =
  ...
  ValoresProcentaje(data) {
    ...
    Object.keys(data).forEach((element) => {
      ...
      Object.keys(data[element]).forEach((etapa) => {
        ...
        auxsumetapa[element][etapa]['num'] = auxsumetapa[element][etapa][
          'num'
        ].toFixed(3);
    ...
    Object.keys(data).forEach((element) => {
      ...
      Object.keys(data[element]).forEach((etapa) => {
        ...
        Object.keys(data[element][etapa]).forEach((subetapa) => {
          ...
          auxsumetapa[element][etapa][subetapa]['num'] = data[element][etapa][
            subetapa
          ].toFixed(3);
